		...
		unsigned int xid_rep, char **b, int *sz)
{
	rpcs::rpcstate_t ret;
        ...
        ScopedLock rwl(&reply_window_m_);
        std::list<reply_t> *tmp_list = &(reply_window_[clt_nonce]);
        if(!tmp_list->empty() && xid < tmp_list->front().xid)
                ret = FORGOTTEN;
        else {
                ...
                if(it == tmp_list->end()) {
                        reply_t rep = reply_t(xid);
                        for(it = tmp_list->begin(); it != tmp_list->end(); it++) {
                                if(it->xid > xid) {
                                        tmp_list->insert(it, rep);
                                        break;
                                }
                        }
                        if(it == tmp_list->end())
                                tmp_list->push_back(rep);

                        ret = NEW;
                }
	}

	while(!tmp_list->empty() && tmp_list->front().xid < xid_rep) {
		free(tmp_list->front().buf);
		tmp_list->pop_front();
	}
