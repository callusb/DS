  ...
  for(it = nodes.begin(); it != nodes.end(); it++) {
    ...
    if(ret != paxos_protocol::OK) {
      ...
      if(r.oldinstance) {
        ...
        tprintf("proposer::prepare: prepare is rejected\n");
    }
  }
  ...
  for(it = nodes.begin(); it != nodes.end(); it++) {
    ...
    if(ret != paxos_protocol::OK) {
      ...
      if(r)
        ...
        tprintf("proposer::accept: accept is rejected\n");
    }
  }
  return;
}
	      ...
	      std::string v)
{
  ...
  for(it = accepts.begin(); it != accepts.end(); it++){
    ...
    tprintf("proposer::decide: decide is sent\n");
  }
  return;
}
