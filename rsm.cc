...
rsm::sync_with_backups()
{
  ...
  std::vector<std::string>::iterator iter = std::find(backups.begin(), backups.end(), primary);
  if(iter != backups.end())
    backups.erase(iter);
  pthread_cond_wait(&recovery_cond, &rsm_mutex);
...
rsm::client_invoke(int procno, std::string req, std::string &r)
{
  ...
  VERIFY(pthread_mutex_lock(&rsm_mutex)==0);
  std::vector<std::string> views = cfg->get_view(vid_commit);
  viewstamp vs;
  last_myvs = myvs;
  vs = last_myvs;
  myvs.seqno++;
...
rsm::invoke(int proc, viewstamp vs, std::string req, int &dummy)
{
  ...
  if(!cfg->ismember(cfg->myaddr(), vs.vid))
    return rsm_protocol::ERR;  
