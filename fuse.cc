                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    std::string buf;
    if(yfs->getfilecon(ino, buf) != yfs_client::OK) {
      fuse_reply_err(req, ENOSYS);
      return;
    }

    if(attr->st_size > info.size) {
      unsigned long long diff = attr->st_size - info.size; 
      buf.append(diff, '\0');
    } else 
                 ...
                 struct fuse_file_info *fi)
{
  ...
  RemoteLock lock(yfs, ino);
  std::string conbuf;
  if(yfs->getfilecon(ino, conbuf) != yfs_client::OK) {
    fuse_reply_err(req, ENOSYS);
    return;
  }
  size_t bufsize = conbuf.size();

  if(off > bufsize) {
    unsigned long long diff = off - bufsize;
    conbuf.append(diff, '\0');
    conbuf.append(buf, size);
  } else if(off + size >= bufsize) {
    conbuf = conbuf.substr(0, off);
    conbuf.append(buf, size);
  } else
    conbuf.replace(off, size, buf, size);

  if(yfs->put(ino, conbuf) != yfs_client::OK)
    fuse_reply_err(req, ENOSYS);
  else
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  // You fill this in for Lab 2a
  if(yfs->isdir(parent)) {
    RemoteLock parlock(yfs, parent);
    std::map<std::string, yfs_client::inum> ents;
    if(yfs->getdircon(parent, ents) != yfs_client::OK)
      return yfs_client::IOERR;

    if(ents.count(name) != 0)
      return yfs_client::EXIST;
    else {
      yfs_client::inum inum = random() | 0x80000000;
      RemoteLock chdlock(yfs, inum);
      if(yfs->create(parent, inum, name) != yfs_client::OK) 
        return yfs_client::IOERR;
      else {
        e->ino = inum;
        return getattr(inum, e->attr);
      }
    }
  } else {
    return yfs_client::IOERR;
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  // You fill this in for Lab 2a
  RemoteLock lock(yfs, inum);
  std::map<std::string, yfs_client::inum> ents;
  if(yfs->getdircon(inum, ents) != yfs_client::OK)
    fuse_reply_err(req, ENOENT);
  else {
     ...
     mode_t mode)
{
  ...
  if(yfs->isdir(parent)) {
    ...
    if(ents.count(name) != 0) {
      ...
      yfs_client::inum inum = random() & 0x7FFFFFFF;
      RemoteLock chdlock(yfs, inum);
      if(yfs->create(parent, inum, name) != yfs_client::OK) {
        fuse_reply_err(req, ENOSYS);
        return;
